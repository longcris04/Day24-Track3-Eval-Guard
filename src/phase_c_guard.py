...
from config import (ADVERSARIAL_SET_PATH, GUARDRAILS_CONFIG_DIR, LATENCY_BUDGET_P95_MS,
                    PRESIDIO_LANGUAGE, OPENAI_API_KEY, OPENROUTER_API_KEY,
                    OPENROUTER_BASE_URL, OPENROUTER_MODEL)

# NeMo Guardrails (qua LangChain) đọc các biến môi trường OPENAI_*. Nếu bạn chỉ có
# OpenRouter key, bắc cầu sang OPENAI_* để NeMo gọi được endpoint OpenAI-compatible
# của OpenRouter. (config.yml dùng model "openai/gpt-4o-mini" — hợp lệ trên OpenRouter.)
...
# Chỉ những entity type liên quan tới HR-PII mới được tính là "có PII".
# Lọc như vậy tránh false-positive từ NER tiếng Anh khi quét văn bản tiếng Việt
# (vd: ngày tháng, tên riêng) — giữ has_pii ổn định cho các test.
PII_ENTITIES = {"VN_CCCD", "VN_PHONE", "EMAIL_ADDRESS", "PHONE_NUMBER"}

# Cache engine để tránh khởi tạo lại Presidio/NeMo trên mỗi lần gọi (rất tốn kém).
...
def setup_nemo_rails():
    ...
    # Nếu chỉ có OpenRouter: NeMo mặc định suy ra LLM kiểu "completion" (class OpenAI) cho
    # tên model lạ → OpenRouter chỉ phục vụ chat endpoint nên call thất bại (0 LLM calls,
    # response rỗng, rail không chặn được gì). Truyền thẳng ChatOpenAI trỏ vào OpenRouter
    # để ép NeMo dùng chat endpoint.
    if not OPENAI_API_KEY and OPENROUTER_API_KEY:
        from langchain_openai import ChatOpenAI
        llm = ChatOpenAI(model=OPENROUTER_MODEL, base_url=OPENROUTER_BASE_URL,
...
def run_adversarial_suite(adversarial_set: list[dict], rails=None,
    ...
    """
    # Khởi tạo engine một lần (cache) để cả 20 inputs dùng chung.
    if analyzer is None or anonymizer is None:
        analyzer, anonymizer = _get_presidio()
    if rails is None:
        rails = _get_rails()
...
if __name__ == "__main__":
    ...
    print(f"\nAdversarial: {passed}/{len(results)} passed ({pass_rate:.0%})")
