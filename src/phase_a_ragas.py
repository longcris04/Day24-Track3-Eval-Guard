...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    def _clean(x: float) -> float:
        """NaN (metric RAGAS tính lỗi) → 0.0 để report luôn là JSON hợp lệ và số sạch."""
        x = float(x)
        return 0.0 if x != x else x  # x != x là True khi x là NaN
    ...
    for a, pq in zip(answers, per_q):
        results.append(RagasResult(
            ...
            faithfulness=_clean(pq.faithfulness), answer_relevancy=_clean(pq.answer_relevancy),
            context_precision=_clean(pq.context_precision), context_recall=_clean(pq.context_recall),
        ))
    return results
...
def bottom_10(results: list[RagasResult]) -> list[dict]:
    ...
    """
    bottom = sorted(results, key=lambda r: r.avg_score)[:10]
    output: list[dict] = []
